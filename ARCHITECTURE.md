...
| [`asset-prep`](crates/asset-prep/) | library (native) | Turns a `dist/` directory into prepared assets + redirect rules + the `state-hash` `Manifest`: directory scanning, MIME detection, content encoding/chunking/hashing, `_headers`/`_redirects` parsing, and html-handling/404 synthesis. One preparation path shared by `sync-core` and the verifier. The compressors are supplied by the caller (`Compressors`); `Compressors::canonical()` is the registry `sync-plugin` and `state-hash-cli` both inject, which is what makes their hashes comparable. |
